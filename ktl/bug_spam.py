                ...
                if should_be_spammed:
                    ...
                    if "comment-text" in self.cfg:
                        ...
                        if not self.cfg["dryrun"]:
                            bug.newMessage(
                                content=self.cfg["comment-text"]
                                .replace("_SERIES_", self.cfg["series"])
                                .replace("_PACKAGE_", self.cfg["package"])
                                .replace("_VERSION_", self.cfg["version"])
                            )
                    ...
                    for tag in (
                        "verification-needed-" + by_package,
                        "kernel-spammed-%s-%s-v2" % (self.cfg["series"], self.cfg["package"]),
                    ):
