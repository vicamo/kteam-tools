...
class CheckComponent:
    ...
    def load_release_components(self, series, package):
        ...
        if rel_ver:
            pkg_rel = archive.getPublishedSources(
                exact_match=True, source_name=package, distro_series=lp_series, pocket="Release", version=rel_ver
            )
            if pkg_rel:
                src_pkg = pkg_rel[0]
    ...
    def override_component(self, default, series, package, bin_pkg):
        ...
        if package == "linux-meta":
            if bin_pkg:
                if (
                    bin_pkg.startswith("linux-backports-modules-")
                    or bin_pkg.startswith("linux-hwe-")
                    or bin_pkg.startswith("linux-image-hwe-")
                ):
    ...
    def name_abi_transform(self, name):
        ...
        if abi:
            ...
            if abi:
                ...
                if version:
                    version = version[0]
                    name = name.replace("%s-%s" % (version, abi), "%s-ABI" % version)
    ...
    def component_function(self, series, package):
        ...
        if (package == "linux") or (package == "linux-signed") or (package == "linux-ppc"):
            ...
            package.startswith("linux-lts-")
            or package.startswith("linux-meta-lts-")
            or package.startswith("linux-signed-lts-")
        ):
            retval = self.main_component
    ...
    def get_published_sources(self, series, package, version, pocket):
        ...
        ps = archive.getPublishedSources(
            exact_match=True, source_name=package, distro_series=lp_series, pocket=pocket.title(), version=version
        )
        if not ps:
    ...
    def components_list(self, series, package, version, pocket, ps=None):
        ...
        if ps:
            src_pkg = ps[0]
            clist.append([src_pkg.source_package_name, src_pkg.source_package_version, src_pkg.component_name])
            for bin_pkg in src_pkg.getPublishedBinaries():
                clist.append([bin_pkg.binary_package_name, bin_pkg.binary_package_version, bin_pkg.component_name])
    ...
    def mismatches_list(self, series, package, version, pocket, ps=None):
        ...
        if ps:
            ...
            if src_pkg.component_name != component:
                ...
                cdebug(
                    "            src_pkg.component_name: %s      component: %s" % (src_pkg.component_name, component),
                    "cyan",
                )
                mlist.append(
                    [src_pkg.source_package_name, src_pkg.source_package_version, src_pkg.component_name, component]
                )
            ...
            for bin_pkg in src_pkg.getPublishedBinaries():
                ...
                if bin_pkg.component_name != component:
                    ...
                    cdebug(
                        "            bin_pkg.component_name: %s      component: %s"
                        % (bin_pkg.component_name, component),
                        "cyan",
                    )
                    mlist.append(
                        [bin_pkg.binary_package_name, bin_pkg.binary_package_version, bin_pkg.component_name, component]
                    )
